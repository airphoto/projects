...
object Features {
  def main(args: Array[String]): Unit = {
    ...
    val dayCondition = " and d>="+args(2)+" and d<="+args(3)//统计时间
    ...
    val array = scala.collection.mutable.ArrayBuffer[String]()//总特征值列表
     ...
     for (i <- columnAndShortList) {//获取所需列的特征值
      ...
      val frame = sqlContext.sql("select distinct(" + column + ") from deliveries where " + column + " is not null and request_time is not null"+dayCondition)
       array ++= frame.map { x => short +"_"+ x.mkString }.collect()//将获取的特征值（前缀_特征值）添加到总特征值列表中
    }
    ...
    sc.makeRDD(tmp).coalesce(10).saveAsTextFile(outPath)//合并到一个文件中存储起来
