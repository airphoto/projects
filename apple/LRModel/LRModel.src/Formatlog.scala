...
object Formatlog {
  def main(args: Array[String]): Unit = {
    ...
    val last = if (args.length == 4) " and d<=" + args(3) else "" //截至时间

    val delSql = "select " + sb.toString() + "click_uuid from deliveries where d>=20151229" + last
    val conSql = "select format_uuid from conversions where d>=20151229" + last
    ...
    val lines = table.drop("click_uuid").map { x =>
      ...
      for (i <- columns) {
        ...
        val tmp = if (fim.keySet.contains(short + x.getString(x.fieldIndex(i)))) " " + fim(short + x.getString(x.fieldIndex(i))) + ":1" else ""
        sb.append(tmp)
      }
      exp + sb.toString()
    }
